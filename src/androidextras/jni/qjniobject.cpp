...
/*!
    ...
    signature matches the function you want to call. The signature stucture is \b \(A\)R, where \b A
    ...
    and clear out the exception before continuing, it is unsafe and to make a JNI call when there are
    unhendled exceptions pending.
    ...
    All objects returned from a JNI call is by default a local references and will only stay valid
    in the scope the function was called. When exiting the scope, the object becomes eligible for
    garbage collection. If you want to keep the object alive you need to either create a new global
